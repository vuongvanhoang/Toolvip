...
def login_tds(token):
	try:
		...
		if 'success' in r:
			os.system('clear')
			print(Colors.green + f"Đăng nhập thành công√\nUser: {Colors.red + r['data']['user'] + Colors.green} | Xu hiện tại: {Colors.red + r['data']['xu']}")
			return 'success'
		else:
...
def check_tiktok(id_tiktok, token):
	try:
		...
		if 'success' in r:
			os.system('clear')
			print(Colors.green + f"{r['data']['msg']}|ID: {Colors.pink + r['data']['id'] + Colors.green}")
			return 'success'
		else:
			print(Colors.red + f"{r['error']}\n")
			return 'error_token'
...
╚══██╔══╝██║  ██║ ╚══██╔══╝██╔══██╗██╔══██╗██║      
   ...
   ╚═╝   ╚═╝  ╚═╝    ╚═╝    ╚════╝  ╚════╝ ╚══════╝ 
      ...
      Admin: Thiệu Hoàng | YouTube: @thieuhoang75     
        Box Zalo: https://zalo.me/g/ahnoav496        
          Ngày: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}               

"""    
gach  = '========================================='

option = f'''{gach}{Colors.red}
Danh sách nhiệm vu: {Colors.green}
1. Follow
2. Tym
{Colors.yellow}{gach}
'''
...
while True:
	try:
		...
		token_tds = Write.Input("Nhập token TDS:", Colors.red_to_green, interval=0.0025)
		cache = 'new'
	for _ in range(3):
	...
	if check_log == 'success':
		if cache == 'old':
			while True:
				print(option_acc)
				try:
					choice = int(Write.Input("Lựa chọn của cậu là:", Colors.red_to_green, interval=0.0025))
					if choice in [1,2]:
						break
					else:
						os.system('clear')
...
if check_log == 'success':
	#Nhập user tiktok
	while True:
		id_tiktok = Write.Input("Nhập ID tiktok:", Colors.yellow_to_red, interval=0.0025)
		for _ in range(3):
		...
		if check_log == 'success':
			...
			os.system('clear')
			print(Colors.yellow + f"ID tiktok chưa được thêm vào cấu hình, vào cấu hình rồi nhập lại!\n")
		else:
			os.system('clear')
	...
	while True:
		print(option)
		try:
			choice = int(Write.Input("Lựa chọn nhiệm vụ muốn làm:", Colors.red_to_green, interval=0.0025))
			if choice in [1,2]:
				break
			else:
				os.system('clear')
	...
	while True:
		try:
			delay = int(Write.Input("Thời gian delay (giây):", Colors.red_to_green, interval=0.0025))
			if delay > 1:
				break
			else:
				os.system('clear')
	...
	while True:
		try:
			max_job = int(Write.Input("Bao nhiêu nhiệm vụ dừng tool:", Colors.red_to_green, interval=0.0025))
			if max_job > 9:
				break
			else:
				os.system('clear')
		...
		if isinstance(list_job, dict) == True:
			for job in list_job['data']:
				...
				if check_duyet != 'error':
					...
					print(f'{Colors.yellow}[{dem_tong}] {Colors.red}| {Colors.cyan}{t_now} {Colors.red}| {Colors.pink}{type_type} {Colors.red}| {Colors.white}{uid}')
					if check_duyet > 9:
						sleep(3)
				...
				if dem_tong == max_job:
					...
					for i in range(delay,-1,-1):
						print(Colors.red + 'THTOOL: '+str(i)+' giây',end=('\r'))
						sleep(1)
		if dem_tong == max_job:
			print(f'{Colors.pink}Hoàn thành {max_job} nhiệm vụ√')
			break
